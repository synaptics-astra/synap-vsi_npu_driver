...
enum {
    /**
      ...
      *
      *  PARAM 0 : TEEC_MEMREF_TEMP_INPUT synap_ta_user_buffer structure describing the buffer
      *            containing the network EBG buffer header, the corresponding buffer will be in
      *            use until this operation returns
      *  PARAM 1 : TEEC_MEMREF_TEMP_INPUT synap_ta_user_buffer structure describing the user
      *            buffer containing the network EBG, the corresponding buffer will be in
      *            use until this operation returns, this buffer must not overlap with the header
      *            buffer
      *  PARAM 2 : TEEC_MEMREF_TEMP_INPUT resources used to run the network -
      *            struct synap_ta_network_resources, the areas listed must be available until
      *            SYNAP_TA_CMD_DESTROY_NETWORK is called
      *  PARAM 3 : TEEC_VALUE_OUTPUT a = nid
      *
      *  NOTE: the EBG file is sent in two parameters because the header must be modifiable only
      *  by the Linux kernel, while the rest of the data can be modified by the user application.
      *  This allows to prevent an attacker with user privileges from making the TA see a different
      *  header than the kernel.
      *
      */
...
struct synap_ta_user_buffer {
    /** offset within the areas where the user buffer starts */
    uint32_t offset;

    /** size of the user buffer*/
    uint32_t size;

    /** number of areas used by the user buffer*/
    uint32_t areas_count;

    /** memory areas occupied of the user buffer */

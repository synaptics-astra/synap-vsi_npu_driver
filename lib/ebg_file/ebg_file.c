...
#define EBG_FILE_LOG_ENTER() SYNAP_TA_LOG_ENTER()
#define EBG_FILE_LOG_SUCCESS() SYNAP_TA_LOG_SUCCESS()
#define EBG_FILE_LOG_FAILURE() SYNAP_TA_LOG_FAILURE()
#define EBG_FILE_LOG(format, ...) SYNAP_TA_LOG(format, ##__VA_ARGS__)
...
bool read_header(uint8_t *addr, struct ebg_file_header *header, uint8_t **next) {
    ...
    if (header->security_type != EBG_FILE_SECURITY_SECURE &&
            header->security_type != EBG_FILE_SECURITY_NO_SECURITY) {
        EBG_FILE_LOG_FAILURE();
        return false;
    }

    header->public_data_length = header->padding_length + header->vsi_data_length +
            header->metadata_length + header->security_info_length + EBG_FILE_HEADER_SIZE;

    header->vsi_data_offset = EBG_FILE_HEADER_SIZE +
            header->security_info_length + header->metadata_length;

    if (header->vsi_data_offset + header->vsi_data_length > header->public_data_length) {
        EBG_FILE_LOG_FAILURE();
        return false;
    }
    ...
    EBG_FILE_LOG("public_data_length %d", header->public_data_length);
...
bool ebg_file_parse_public_data(uint8_t *ebg, size_t len, struct ebg_file_public_data* public_data) {
    ...
    if (len < public_data->header.public_data_length) {
        EBG_FILE_LOG("expected at least %d bytes, received %d",
                     (uint32_t) public_data->header.public_data_length, (uint32_t) len);
        EBG_FILE_LOG_FAILURE();
        return false;
    }
    ...
    public_data->public_data_len = public_data->header.public_data_length;
...
bool ebg_file_parse_vsi_data_header(uint8_t* ebg_file_head, size_t ebg_file_head_size,
    ...
    if (pd->header.vsi_data_offset + pd->header.vsi_data_length > ebg_file_head_size) {
        EBG_FILE_LOG_FAILURE();
        return false;
    }

    p = ebg_file_head + pd->header.vsi_data_offset;

    if (pd->header.vsi_data_length < EBG_FILE_VSI_DATA_HEADER_SIZE) {
        EBG_FILE_LOG_FAILURE();
        return false;
    }
    ...
    vsi_data->total_data_length = EBG_FILE_VSI_DATA_HEADER_SIZE;
    vsi_data->total_data_length += vsi_data->init_realloc_count * EBG_FILE_REALLOC_TABLE_SIZE;
    vsi_data->total_data_length += vsi_data->code_realloc_count * EBG_FILE_REALLOC_TABLE_SIZE;
    vsi_data->total_data_length += vsi_data->layer_count * EBG_FILE_LAYER_TABLE_SIZE;
    vsi_data->total_data_length += vsi_data->operation_count * EBG_FILE_OPERATION_TABLE_SIZE;

    if (vsi_data->total_data_length > pd->header.vsi_data_length) {
        EBG_FILE_LOG_FAILURE();
        return false;
    }
...
bool ebg_file_parse_layers(uint8_t* ebg_file_head, size_t ebg_file_head_size,
    ...
    if (vsi_data->total_data_length + pd->header.vsi_data_offset > ebg_file_head_size) {
        EBG_FILE_LOG_FAILURE();
        return false;
    }

    p = ebg_file_head + pd->header.vsi_data_offset + EBG_FILE_VSI_DATA_HEADER_SIZE;

    // skip realloc tables
    p += vsi_data->init_realloc_count * EBG_FILE_REALLOC_TABLE_SIZE;
    p += vsi_data->code_realloc_count * EBG_FILE_REALLOC_TABLE_SIZE;
...
bool ebg_file_parse_operations(uint8_t* ebg_file_head, size_t ebg_file_head_size,
    ...
    p += vsi_data->init_realloc_count * EBG_FILE_REALLOC_TABLE_SIZE;
    p += vsi_data->code_realloc_count * EBG_FILE_REALLOC_TABLE_SIZE;
    p += vsi_data->layer_count * EBG_FILE_LAYER_TABLE_SIZE;
